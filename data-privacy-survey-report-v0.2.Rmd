...
```{r readdata}
# TO DO: write code that looks for files in both pseud and processed folders 
# and picks the latest one

# Find most recent data file in data/pseud folder
data_files <- list.files(path = "data/pseud")
...
## One-on-one meetings
Besides the survey, we aimed to organise one-on-one meetings with researchers, to hear more in-depth experiences, challenges and needs concerning handling personal data in research. Survey respondents could voluntarily leave their email address at the end of the survey to be contacted by us.

From the survey respondents, `r length(dppsurvey$Email_1_TEXT[!is.na(dppsurvey$Email_1_TEXT)])` researchers left their email address to be contacted. Of those, 22 replied to our email asking to meet up. So far, 18 one-on-one, mostly online, meetings of have been conducted, each taking about 30 minutes. During the meetings, at least one of the project coordinators was present, together with either the other project coordinator or the relevant faculty privacy officer. 

Below, the division over faculties can be seen for all respondents that left their email address:

```{r emailaddressplot}
# Simple barplot of faculty division - Need to drop NA
emailsplot <- 
  dppsurvey %>% 
  filter(Email == "Yes") %>%
  pivot_longer(cols = grep("Faculty_[0-9]$", names(dppsurvey), value=TRUE), 
  ...
  geom_bar(stat = "identity") + 
  labs(x = "", title = "Faculties",
       caption = "Faculties of those who left their email address") +
  coord_flip() +
  theme_classic() +
  geom_label(aes(label = Count), position=position_dodge(width=1.2),
             color = "black",
...
positionemailplot <-
  ...
  ggplot(aes(x = Position, y = Count)) + 
  geom_bar(stat = "identity") + 
  labs(x = "", title = "Positions",
       caption = "Positions of those who left their email address") +
  coord_flip() +
  theme_classic() + 
  geom_label(aes(label = Count), position=position_dodge(width=1.2),
             color = "black",
...
grid.arrange(emailsplot, positionemailplot, nrow = 1)
```
...
kables(list(
  kable(datatypetable, 
        ...
        align = "l",
        caption = "Types of data used among survey respondents"),
  kable(tablepersdata, 
        col.names = gsub("_", " ", names(tablepersdata)), 
        align = "l",
        caption = "Types of personal data used among survey respondents")))
