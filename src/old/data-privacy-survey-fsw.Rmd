...
subtitle: "Results for the faculty of Social and Behavioural Sciences"
author:
- "Dorien Huijser"
...
abstract: |
  ...
  report summarises the results from respondents from the faculty of Social and 
...
the faculty of Social and Behavioural Sciences, a faculty where most research
is performed among humans and thus contains personal data. 

**NOTE: in this report only the graphs have been generated, the text still
has to be adapted**

# Methods {#methods}
The full methods are described in the UU-wide report. Below is a summary of the
details specific to the faculty of Social and Behavioural Sciences (FSBS).
...
There were `r dim(dppsurvey)[1]` respondents from the faculty of  in the 
...
dppsurvey %>%
  ...
  labs(x = "", 
       ...
       caption = "Survey respondents per department and position in FSBS") +
...
researchers from FSBS left their email address to be contacted,
...
conducted, each taking about 30 minutes. 
...
fsw_datatypegraphs <- personaldatatypes %>%
  pivot_longer(cols = grep("^Dept_FSW_[0-9]+$", names(dppsurvey), value=TRUE), 
               values_drop_na = TRUE,
               values_to = "Department") %>%
  select(-name) %>%
...
knowpoplot <- dppsurvey %>% 
  ...
  labs(x = "", 
       ...
       caption = "Awareness of the privacy officer across FSBS departments") +
  coord_flip() +
  theme_classic() +
