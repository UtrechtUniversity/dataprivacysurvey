...
- The full survey
[in text](https://utrechtuniversity.github.io/dataprivacysurvey/documentation/survey-questions-qualtrics.pdf){target="_blank"}
and [as Qualtrics export](https://utrechtuniversity.github.io/dataprivacysurvey/documentation/Data_Privacy_Survey_Qualtrics_export.qsf){target="_blank"}.
...
As the original dataset contains personal information (demographic information, open text 
responses, email addresses, etc.), and no consent was obtained to share those 
details, we are unable to share them in this repository. We did however create
[dummy data files](https://github.com/UtrechtUniversity/dataprivacysurvey/tree/main/data/processed){target="_blank"}
using Qualtrics's "Generate responses" functionality (for the fake raw survey data)
and the website "[Mockaroo.com](https://www.mockaroo.com/){target="_blank"}" 
(for the interview and open text response data). These files can be used to 
regenerate the current report, but will not create any realistic results. 

To reproduce this report:

1. Clone [the repository](https://github.com/UtrechtUniversity/dataprivacysurvey){target="_blank"} from GitHub.
2. In RStudio, open the file [plot-data.R](https://github.com/UtrechtUniversity/dataprivacysurvey/blob/main/src/plot-data.R){target="_blank"}
3. In the first `dependencies` block of `plot-data.R`, change `reproduce <- "no"` to 
`reproduce <- "yes"` so that the fake data files are read into the report.
4. Open the file [data-privacy-survey-report.Rmd](https://github.com/UtrechtUniversity/dataprivacysurvey/blob/main/src/data-privacy-survey-report.Rmd){target="_blank"}
and knit the document into a "fake" survey report.
