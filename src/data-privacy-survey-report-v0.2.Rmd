...
### Survey questions 
The online survey was created by the project coordinators (DH, NM) with 
input from a wide variety of experts, consisting of privacy officers, data
managers, data consultants, information security officers and research engineers.
...
### Survey respondents
The survey was filled out by `r dim(dppsurvey)[1]` UU researchers. We received 
responses from each faculty, but the distribution was not equal: 
the faculties of Science and Social and Behavioural Sciences were 
...
a large chunk of research performed at the Faculty of Geosciences may not concern
data from people (i.e. not much personal data). The Faculty of Medicine, then, is 
located at the University Medical Center Utrecht (UMCU) and is not of primary 
interest for the current survey, which was sent to UU researchers only.

As can be seen in the figure on the right, most respondents were relatively early 
...
position_per_faculty <- dppsurvey %>%
  ...
  geom_label(aes(label = after_stat(y), group = Faculty), 
             stat = 'summary', fun = sum, vjust = 0.5, hjust = "inward", #-0.2,
             label.size = NA, #1,
             color = "black",
             fill = "#FFFFFF") + #"#FFCD00") +
...
addresses). The open text responses were separately and manually scored in concert
with the notes from the one-on-one meetings with researchers (see below). 
...
- The <a href="https://github.com/UtrechtUniversity/dataprivacysurvey/tree/main/src" target = "_blank">scripts used to clean the data and write this report</a>.
...
## One-on-one meetings
Besides the survey, we organised one-on-one meetings with researchers, to hear 
about their personal experiences, challenges and needs concerning the handling 
of personal data in their research. Survey respondents could voluntarily leave 
their email address at the end of the survey to be contacted by us.
...
indicated that they were willing to meet with us. So far, 27 one-on-one online 
...
other project coordinator (NM) or the relevant faculty privacy officer. 

Below, the division over faculties can be seen for all respondents that left their email address:
